...
export const useHyperStore = defineStore('hyper', () => {
  ...
  const moduleVersion = ref('v6.8.4')
...
echo "VER:$(grep '^version=' /data/adb/modules/hypercore/module.prop 2>/dev/null | cut -d= -f2 || echo v6.8.4)";
echo "===GL===";
    ...
    try {
      ...
      if (kv.IPC && kv.IPC.includes('"status":"ok"')) {
        try {
          ...
          if (typeof ipcData.thermal_tier !== 'undefined') {
            const tierNames = ['Optimal', 'Warm Mitigation', 'Hot Protection']
            thermalTier.value = tierNames[ipcData.thermal_tier] || `Tier ${ipcData.thermal_tier}`
          }
